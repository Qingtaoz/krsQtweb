layui.use(['layer', 'element', 'util'], function () {
     ...
     * @param elem
     */
    function tabSubMenu(elem){
        var menuid;
        if (!elem.attr('data-menuid')){
            return;
        }
        menuid = elem.attr('data-menuid');
        $(document).find(".layui-nav[lay-filter='side'] div.submenus").each(function () {
    ...
    function buildSubMenu(){
        if ($(window).width() < 1024) {
            ...
            var elem = $(document).find(".layui-nav[lay-filter='side-left'] li.layui-this")
            var menuid;
            if (!elem.attr('data-menuid')){
                return;
            }
